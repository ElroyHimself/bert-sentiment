...
class TweetDataset:
    ...
    def __getitem__(self, item):
        ...
        #tok_type_ids = [0] * 3 + [1] * (len(tok_tweet_ids) - 3)
        tok_type_ids =[0] * len(tok_tweet_ids)
        padding_length = self.max_len - len(tok_tweet_ids)
        ids = tok_tweet_ids + ([0] * padding_length)
        mask = mask + ([0] * padding_length)
        tok_type_ids = tok_type_ids + ([0] * padding_length)
        ...
        return {
            ...
            'tok_type_ids': torch.tensor(tok_type_ids, dtype=torch.long),
...
class TweetDataset:
    ...
    def __getitem__(self, item):
        ...
        sentiment = [1, 0, 0]
        if self.sentiment[item] == 'postive':
            sentiment = [0, 0, 1]
        if self.sentiment[item] == 'negative':
            sentiment = [0, 1, 0]
        ...
        return {
            ...
            "tweet_tokens": " ".join(tok_tweet_tokens),
            "otig_tweet": self.tweet[item],
            "sentiment": torch.tensor(sentiment, dtype=torch.long),
            "orig_sentiment": self.sentiment[item],
            "orig_slected": self.selected_text[item]
        }
